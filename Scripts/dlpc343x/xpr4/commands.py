...
clr.AddReference('DLPComposer.Commands.DLPC343x.XPR4')
from System import *
from DLPComposer.Commands.DLPC343x.XPR4 import * 
...
def WriteActuatorFixedSteppedOutputInvertEnable(FixedOutputInvertEnable, SteppedOutputInvertEnable) :
    return Command.WriteActuatorFixedSteppedOutputInvertEnable(FixedOutputInvertEnable, SteppedOutputInvertEnable)

def ReadActuatorFixedSteppedOutputInvertEnable() :
    Summary, FixedOutputInvertEnable, SteppedOutputInvertEnable = \
        Command.ReadActuatorFixedSteppedOutputInvertEnable()
    return Summary, FixedOutputInvertEnable, SteppedOutputInvertEnable
...
def WriteActuatorGlobalLatency(SyncLatencyValue, ManualLatencyEnable, AutoLatencyScalingEnable) :
    return Command.WriteActuatorGlobalLatency(SyncLatencyValue, ManualLatencyEnable, AutoLatencyScalingEnable)

def ReadActuatorGlobalLatency() :
    Summary, SyncLatencyValue, ManualLatencyEnable, AutoLatencyScalingEnable = \
        Command.ReadActuatorGlobalLatency()
    return Summary, int(SyncLatencyValue), ManualLatencyEnable, AutoLatencyScalingEnable

def WriteActuatorGlobalClockStretch(PeriodStretch) :
    return Command.WriteActuatorGlobalClockStretch(PeriodStretch)

def ReadActuatorGlobalClockStretch() :
    Summary, PeriodStretch = \
        Command.ReadActuatorGlobalClockStretch()
    return Summary, int(PeriodStretch)

def WriteActuatorGlobalSubframeFilterWidth(ActuatorSubframeFilterWidth12, ActuatorSubframeFilterWidth34) :
    return Command.WriteActuatorGlobalSubframeFilterWidth(ActuatorSubframeFilterWidth12, ActuatorSubframeFilterWidth34)

def ReadActuatorGlobalSubframeFilterWidth() :
    Summary, ActuatorSubframeFilterWidth12, ActuatorSubframeFilterWidth34 = \
        Command.ReadActuatorGlobalSubframeFilterWidth()
    return Summary, int(ActuatorSubframeFilterWidth12), int(ActuatorSubframeFilterWidth34)

def WriteActuatorGlobalWatchdogWindowWidth(ActuatorWatchdogWindowWidth12, ActuatorWatchdogWindowWidth34) :
    return Command.WriteActuatorGlobalWatchdogWindowWidth(ActuatorWatchdogWindowWidth12, ActuatorWatchdogWindowWidth34)

def ReadActuatorGlobalWatchdogWindowWidth() :
    Summary, ActuatorWatchdogWindowWidth12, ActuatorWatchdogWindowWidth34 = \
        Command.ReadActuatorGlobalWatchdogWindowWidth()
    return Summary, int(ActuatorWatchdogWindowWidth12), int(ActuatorWatchdogWindowWidth34)

def WriteActuatorGlobalDacOutputEnable(ActuatorDacOutputEnable) :
    return Command.WriteActuatorGlobalDacOutputEnable(ActuatorDacOutputEnable)

def ReadActuatorGlobalDacOutputEnable() :
    Summary, ActuatorDacOutputEnable = \
        Command.ReadActuatorGlobalDacOutputEnable()
    return Summary, ActuatorDacOutputEnable

def WriteActuatorOrientation(DecodeOrder, SubframeOrder, HFlipMap, VFlipMap, BFlipMap) :
    return Command.WriteActuatorOrientation(DecodeOrder, SubframeOrder, HFlipMap, VFlipMap, BFlipMap)

def ReadActuatorOrientation() :
    Summary, DecodeOrder, SubframeOrder, HFlipMap, VFlipMap, BFlipMap = \
        Command.ReadActuatorOrientation()
    return Summary, int(DecodeOrder), int(SubframeOrder), int(HFlipMap), int(VFlipMap), int(BFlipMap)
