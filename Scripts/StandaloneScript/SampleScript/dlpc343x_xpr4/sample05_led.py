###########################################################################################################################################################################
# Texas Instruments DLP LightCrafter 230NP EVM Python Support Code - sample05_led.py - Last Updated May 8th, 2020
...
def main():
        ...
        DLPDLCR230NPEVM LED Current Information is as follows:
            > Minimum IDAC Value: 51 (of 1023)
            > Maximum IDAC Value: 810 (of 1023)
            > Nominal IDAC Value  610 (of 1023)
        ...
        for led_dac_value in range(2,27):
            Summary = WriteRgbLedCurrent(led_dac_value*30,60,60)
            time.sleep(1)

        print("Testing GREEN LED...")
        Summary = WriteRgbLedEnable(0,1,0)
        for led_dac_value in range(2,27):
            Summary = WriteRgbLedCurrent(60,led_dac_value*30,60)
            time.sleep(1)

        print("Testing BLUE LED...")
        Summary = WriteRgbLedEnable(0,0,1)
        for led_dac_value in range(2,27):
            Summary = WriteRgbLedCurrent(60,60,led_dac_value*30)
            time.sleep(1)

        Summary = WriteRgbLedEnable(1,1,1)
        Summary = WriteRgbLedCurrent(610,610,610)
        time.sleep(1)
