###########################################################################################################################################################################
# Texas Instruments DLP LightCrafter 230NP EVM Python Support Code - internal02_transition_mode.py - Last Updated May 8th, 2020
...
def main():
        '''
        This script cycles through available display sources on the DLPDLCR230NPEVM.
        Raspberry Pi automatically configures each source via I2C.
        ...
        input("Press any KEY to continue EXTERNAL...")
        ...
        Summary = WriteInputImageSize(1920, 1080)
        time.sleep(delay)
        Summary = WriteSplashScreenSelect(0)
        time.sleep(delay)
        Summary = WriteSourceSelect(Source.SplashScreen, Set.Disabled)
        if curtain_enable:
                Summary = WriteDisplayImageCurtain(0,Color.Black)
        input("Press any KEY to continue SPLASH...")
        ...
        if curtain_enable:
                Summary = WriteDisplayImageCurtain(0,Color.Black)
        input("Press any KEY to continue FPGATPG...")
        ...
        if curtain_enable:
                Summary = WriteDisplayImageCurtain(1,Color.Black)
        Summary = WriteSplashScreenSelect(1)
        time.sleep(delay)
        Summary = WriteSourceSelect(Source.SplashScreen, Set.Disabled)
        if curtain_enable:
                Summary = WriteDisplayImageCurtain(0,Color.Black)
