###########################################################################################################################################################################
# Texas Instruments DLP LightCrafter 230NP EVM Python Support Code -  internal01_projon.py - Last Updated May 8th, 2020
...
def main():
        '''
        This script allows for a power-cycle test of the system. 
        The PROJ_ON signal which controllers power state of the DLPDLCR230NP EVM is toggled on and off to perform this test. 
        Note that sufficient minimum time should be allowed between power cycles so that the entire system can boot up before shutting back down again.
        Recommended 3 seconds power-up time given when booting the DLPDLCR230NP EVM
        '''
        ...
        print("Cycling PROJ_ON for DLPC3436...")
        time.sleep(3)

        for cycle_count in range(0,10+1):
            os.system("raspi-gpio set 26 op dh")
            print("PROJ_ON --> OFF...")
            time.sleep(5)
            os.system("raspi-gpio set 26 op dl")
            print("PROJ_ON --> ON...")
            time.sleep(8)
