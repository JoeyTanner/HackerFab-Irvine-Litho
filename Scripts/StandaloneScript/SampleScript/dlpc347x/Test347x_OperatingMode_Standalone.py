...
def configureForSplash(index, dmdPixels, dmdLines):
    ...
    first_pixel = 0
    first_line = 0
    Summary = WriteDisplaySize(first_pixel, first_line, DmdWidth, DmdHeight)
    if not Summary.Successful:
...
def main():
    with Cypress(0x36) as dev, LightCrafterDisplay(dev) as lcd:
        ...
        # Get the current operating mode and save for later restoration
...
# main program for t_0x05_0x06_OperatingMode.py
