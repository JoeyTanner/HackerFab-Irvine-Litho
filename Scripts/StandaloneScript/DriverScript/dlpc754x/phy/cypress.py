...
#
#  This module uses ctypes to implement an interface to the Cypress I2C
...
class Cypress(implements(PhysicalLayer)):
    ...
    def __init__(self, slave=0x34, devtype=CY_DEVICE_TYPE.CY_TYPE_I2C, product=None):
    ...
    def _identify(self):
        count = self.GetListofDevices()
        if count == 0:
            utils.print_error("ERROR: Could not find any Cypress device.")

        # Only show I2C devices
        self.i2c_device_instance = []
        for i in xrange(0, count):
            if self.GetDeviceInfo(i).deviceType[0] == CY_DEVICE_TYPE.CY_TYPE_I2C:
                self.i2c_device_instance.append(i)

        if utils.is_debug():
            for i in xrange(0, len(self.i2c_device_instance)):
                utils.print_and_flush("{0} - Cypress I2C Device\r\n".format(i))
    ...
    def get_type(self):
        if self.devtype == CY_DEVICE_TYPE.CY_TYPE_I2C:
            ...
            # SPI is not yet supported
            return None

    def connect(self, instance = 0):
        if instance >= self.GetListofDevices():
            utils.print_error("ERROR: Opening Cypress instance {0} failed.".format(instance))
            return False

        self.instance = self.i2c_device_instance[instance]
        self._open()
        ...
        if self.print_event_msg or utils.is_debug():
            utils.print_and_flush("Opened Cypress Interface\r\n")
        return True

    def disconnect(self):
        self._close()
        if self.print_event_msg or utils.is_debug():
            utils.debug_print("Closed Cypress Interface\r\n")
    ...
    def send(self, byte_arr):
        try:
            ...
            utils.debug_print("Resetting Cypress device...")
            self.reset()
    ...
    def receive(self, read_len, timeout_in_sec = 2):
        try:
            self.timeout = timeout_in_sec * 1000
            # First see if we have a buffer containing unread bytes
            unread_bytes = len(self.unread_data)
            utils.debug_print("\r\nB4: {0}\r\n".format(utils.byte_array_to_hex_str(self.unread_data)))
            if unread_bytes < read_len:
                # Find out no of bytes to read
                num_bytes_to_read = read_len - unread_bytes
                num_bytes_to_read = 8 *  ((num_bytes_to_read + 7) // 8)
                utils.debug_print("RL = {0}, UR = {1}, NBR = {2}\r\n".format(read_len, unread_bytes, num_bytes_to_read))
                self.unread_data += self._read(num_bytes_to_read)
                utils.debug_print("AR: {0}\r\n".format(utils.byte_array_to_hex_str(self.unread_data)))
            data = self.unread_data[:read_len]
            del self.unread_data[:read_len]
        except CypressError as e:
            utils.print_error("ERROR: {0}".format(e))
            # Do a reset to see if we can recover
            utils.debug_print("Resetting Cypress device...")
    ...
    def _write(self, sendbytes, bytes_to_read=0, slave=None):
    ...
    def set_speed(self, speed_in_khz):
        """ set the current I2C bus speed in KHz used by Cypress """
        if self.handle is None:
            raise ValueError("Device not open (call open() or 'with Cypress()')")
        if self.devtype == CY_DEVICE_TYPE.CY_TYPE_I2C:
    ...
    def SetI2cConfig(self, handle, freq, slaveAddr, master, stretch):
        if self.dll is None:
            self.load()
        cfg = self.CYI2CCONFIG(freq, slaveAddr, master, stretch)
        status = self.dll.CySetI2cConfig(handle, byref(cfg))
        if status:
            raise CypressError(status)

    def I2cRead(self, handle, slave_addr, bytes_to_read, timeout=1000):
        if self.dll is None:
            self.load()
    ...
    def I2cWrite(self, handle, slave_addr, sendbytes, timeout=1000):
        if self.dll is None:
            self.load()
        datacfg = self.CYI2CDATACONFIG(slave_addr >> 1, 0, 0)
    ...
    def SetSpiConfig(self, handle, frequency, dataWidth, protocol, isMsbFirst,
                     isMaster, isContinuousMode, isSelectPrecede, isCpha, isCpol):
        if self.dll is None:
            self.load()
        cfg = self.CYSPICONFIG(frequency, dataWidth, protocol, isMsbFirst,
                               isMaster, isContinuousMode, isSelectPrecede,
                               isCpha, isCpol)
