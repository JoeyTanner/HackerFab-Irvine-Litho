...
def spi_connect(dev_instance = 0):
    _disconnect()
    available_spi_interfaces = SPICommandInterface.GetAvailableInterfaces()
    if len(available_spi_interfaces) < (dev_instance + 1):
        return False    
    spi_interface = available_spi_interfaces[dev_instance]
    return _connect(spi_interface)

__all__ = ['usb_connect', 'i2c_connect', 'spi_connect']